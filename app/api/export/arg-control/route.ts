...
export async function GET(req: Request) {
  try {
    ...
    if (scope === "org") {
      ...
      // Excel-Sheetnamen müssen innerhalb eines Workbooks eindeutig sein —
      // zwei Mitarbeitende mit identischem Vor-/Nachnamen (in echten
      // Organisationen nicht ausgeschlossen) würden addWorksheet() sonst
      // zum Absturz bringen. Bei einer Kollision wird der Sheetname
      // durchnummeriert.
      const usedSheetNames = new Set<string>();
      for (const m of memberships) {
        const data = await loadPersonData(orgId, m.userId, startDate, endDate);
        if (!data) continue;
        let sheetName = data.name.slice(0, 31); // Excel-Limit: max. 31 Zeichen je Sheetname
        let suffix = 2;
        while (usedSheetNames.has(sheetName)) {
          const marker = ` (${suffix})`;
          sheetName = data.name.slice(0, 31 - marker.length) + marker;
          suffix++;
        }
